...
void buildGraph(ifstream& infile, list<struct Graph>& countryList){
    ...
    cout << endl;
    cout<< "Number of Countries in ServerA:" << countryList.size() <<endl;
        ...
        for(auto graph : countryList){
            if (graph.country == country){
                if(graph.userIdToIndex.find(userId) == graph.userIdToIndex.end()){
                    ...
                    str.copy(send_buf,str.length(),0);
                    send_buf[str.length()] = '\0';
                    cout<< "The server A has sent \"User " << userIdString << " not found\" to Main Server" << endl;
                }else {
                    int recomId = getRecomd(graph,userId);
                    cout<< "The server A is searching possible friends for User" << userIdString << " …"<< endl;
                    if (recomId < 0) {
...
int UDP(list<struct Graph>& countryList){
    cout<< "The server A is up and running using UDP on port " << MY_PORT <<endl;
